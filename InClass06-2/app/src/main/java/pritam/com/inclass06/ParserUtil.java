            ...
            while (event!=XmlPullParser.END_DOCUMENT)
            {
                switch (event){
                    case XmlPullParser.START_TAG:
                        ...
                        {
                            app=new App();
                            app.setId(Integer.parseInt(parser.getAttributeValue(null,"id")));
                            //String url = parser.getAttributeValue(null,"href").trim();
                            Log.d("Demo", app.getId()+"");
                        ...
                        else if(parser.getName().equalsIgnoreCase("price"))
                        {
                            ...
                            //Log.d("Demo","$$$$"+price);
                            app.setAnd_app_price(price.substring(0,price.length()-3));
                        }
                        ...
                        {
                            String releaseDate = parser.getAttributeValue(null,"label").trim();
                            app.setReleaseDate(releaseDate);
                        }
                        else if(parser.getName().equalsIgnoreCase("category"))
                        {
                            String category = parser.getAttributeValue(null,"label").trim();
