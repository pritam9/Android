  ...
  $.ajax(
      {
          ...
          type: "POST",
          crossDomain: true,
          data: "token="+token+"&caregiver="+caregiverId+"&description="+description,
          error: function(e){
              console.log(e);
              alert("error - "+e);
          },
...
function alertResponse(msg,sender,reqId){
  requestId =reqId;
  respondToBoomerId=sender;
  document.getElementById("caregiver_details").innerHTML=msg;
  alert(sender+ " - "+msg+" - "+reqId);
  $("#join_caregiver").toggle();
}
function respondToRequest(status){
    if(status==='ACCEPT'){
        var description = $("#requirements").val();
          $.ajax(
              {
                  url:"http://ec2-35-166-132-223.us-west-2.compute.amazonaws.com/boomer/req/caregiver/confirm.do",
                  //url:"http://localhost:8080/springmvc/login",
                  type: "POST",
                  crossDomain: true,
                  data: "token="+token+"&boomer="+respondToBoomerId+"&description="+description+"&requestId="+requestId,
                  error: function(e){
                      console.log(e);
                      alert("error - "+e);
                  },
        ...
        var description = $("#requirements").val();
                  $.ajax(
                      {
                          url:"http://ec2-35-166-132-223.us-west-2.compute.amazonaws.com/boomer/req/accept.do",
                          type: "POST",
                          crossDomain: true,
                          data: "token="+token+"&boomer="+respondToBoomerId+"&description="+description+"&requestId="+requestId,
                          error: function(e){
           ...
           var description = $("#requirements").val();
             $.ajax(
                 {
                     url:"http://ec2-35-166-132-223.us-west-2.compute.amazonaws.com/boomer/req/caregiver/cancel.do",
                     type: "POST",
                     crossDomain: true,
                     data: "token="+token+"&boomer="+respondToBoomerId+"&description="+description+"&requestId="+requestId,
                     error: function(e){
